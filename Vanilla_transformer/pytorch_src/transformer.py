...
class TransformerEncoderLayer(Module):
    ...
    def forward(
        ...
        if self.norm_first:
            ...
            sablock_op = self._sa_block(x, src_mask, src_key_padding_mask)

            print("x + self_atten(x) = ")
            print(x + sablock_op)
            print()

            x = self.norm1(x + sablock_op)

            # x = self.norm1(x + self._sa_block(x, src_mask, src_key_padding_mask, is_causal=is_causal))
            print("norm1(x + self_atten(x)) = ")
            print(x)
            print()

            # x = self.norm2(x + self._ff_block(x))

            ff_op = self._ff_block(x)
            ######################
            print("feed_fwd_op = ", ff_op)
            print()
            ######################
            x = self.norm2(x + ff_op)
            #####################
...
class TransformerDecoderLayer(Module):
    ...
    def forward(
        ...
        if self.norm_first:
            ...
            ########################
            print("Decoder norm2(x + mha(x, mem)) = ")
            print(x)
            print()
            ########################
